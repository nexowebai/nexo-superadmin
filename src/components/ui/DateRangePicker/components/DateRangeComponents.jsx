...
export function CalendarGrid({ monthDate, startDate, endDate, hoverDate, onDateClick, onDateHover }) {
  ...
  return (
    <div className="drp-calendar">
      ...
      <div className="drp-weekdays">{weekDays.map(d => <div key={d} className="drp-weekday">{d}</div>)}</div>
      <div className="drp-days">
        {days.map((day, idx) => {
          ...
          if (startDate && !endDate && hoverDate) {
            const [s, e] = startDate < hoverDate ? [startDate, hoverDate] : [hoverDate, startDate];
            isHoverRange = isWithinInterval(day, { start: s, end: e }) && !isSameDay(day, startDate);
          }
          return (
            <button key={idx} onClick={() => onDateClick(day)} onMouseEnter={() => onDateHover(day)} disabled={!isCurrentMonth} type="button"
                    className={cn("drp-day", !isCurrentMonth && "outside", isSelectedStart && "selected start", isSelectedEnd && "selected end", isInRange && "in-range", isHoverRange && "in-hover-range")}>
              {format(day, "d")}
            </button>
          );
        })}
