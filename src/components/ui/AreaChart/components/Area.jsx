...
export function Area({
    ...
    animate = true, showLine = true, showHighlight = true,
    gradientToOpacity = 0, fadeEdges = false,
}) {
    const {
    ...
    const findLengthAtX = useCallback((targetX) => {
    ...
    return (
        <>
            <defs>
                ...
                {fadeEdges && (
                    <>
                        ...
                        <mask id={edgeMaskId}><rect fill={`url(#${edgeGradientId})`} height={innerHeight} width={innerWidth} x="0" y="0" /></mask>
                    </>
                )}
                {animate && (
                    <clipPath id={`grow-clip-area-${dataKey}`}>
                        <rect height={innerHeight + 20} y={0} x={0}
            ...
            <g clipPath={animate ? `url(#grow-clip-area-${dataKey})` : undefined}>
                <motion.g animate={{ opacity: isHovering && showHighlight ? 0.6 : 1 }} initial={{ opacity: 1 }}>
                    ...
                    {showLine && (
                        <LinePath curve={curve} data={data} innerRef={pathRef} stroke={`url(#${strokeGradientId})`}
                            strokeLinecap="round" strokeWidth={strokeWidth} x={(d) => xScale(xAccessor(d)) ?? 0} y={getY} />
                    )}
                    {data.map((d, i) => (
                        <circle key={i} cx={xScale(xAccessor(d))} cy={getY(d)} r={3} fill={resolvedStroke} stroke="#fff" strokeWidth={1} className="transition-all duration-300 hover:r-4" />
                    ))}
                </motion.g>
            </g>

            {showHighlight && showLine && isHovering && isLoaded && pathRef.current && (
                <motion.path animate={{ opacity: 1 }} d={pathRef.current.getAttribute("d") || ""} exit={{ opacity: 0 }} initial={{ opacity: 0 }}
                    stroke={resolvedStroke} strokeLinecap="round" strokeWidth={strokeWidth} transition={{ duration: 0.4 }}
