...
function extractAreaConfigs(children) {
    ...
    Children.forEach(children, (child) => {
        ...
        const isArea = name === "Area" || child.type === Area || (props?.dataKey?.length > 0);
        if (isArea && props?.dataKey) configs.push({ dataKey: props.dataKey, stroke: props.stroke || props.fill || "var(--chart-line-primary)", strokeWidth: props.strokeWidth || 2 });
    });
    return configs;
}
...
function ChartInner({ width, height, data, xDataKey, margin, animationDuration, children, containerRef }) {
    ...
    const xAccessor = useCallback(d => d[xDataKey] instanceof Date ? d[xDataKey] : new Date(d[xDataKey]), [xDataKey]);
    const bisectDate = useMemo(() => bisector(d => xAccessor(d)).left, [xAccessor]);
    const xScale = useMemo(() => scaleTime({ range: [0, innerWidth], domain: [Math.min(...data.map(d => xAccessor(d).getTime())), Math.max(...data.map(d => xAccessor(d).getTime()))] }), [innerWidth, data, xAccessor]);
    const yScale = useMemo(() => {
        let maxValue = Math.max(...lines.flatMap(l => data.map(d => d[l.dataKey]).filter(v => typeof v === 'number'))) || 100;
    ...
    const preOverlay = [], postOverlay = [];
    Children.forEach(children, child => isPostOverlayComponent(child) ? postOverlay.push(child) : preOverlay.push(child));

    const contextValue = { data, xScale, yScale, width, height, innerWidth, innerHeight, margin, columnWidth: innerWidth / (data.length - 1), tooltipData, setTooltipData, containerRef, lines, isLoaded, animationDuration, xAccessor, dateLabels, selection, clearSelection };

    return (
        <ChartProvider value={contextValue}>
            <svg aria-hidden="true" height={height} width={width}>
                <defs><clipPath id="chart-area-grow-clip"><rect height={innerHeight + 20} width={isLoaded ? innerWidth : 0} style={{ transition: isLoaded ? "none" : `width ${animationDuration}ms cubic-bezier(0.85, 0, 0.15, 1)` }} /></clipPath></defs>
...
export function AreaChart({ data, xDataKey = "date", margin: marginProp, animationDuration = 1100, aspectRatio = "2 / 1", className = "", children }) {
