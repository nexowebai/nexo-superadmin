...
export function useAdminsPage() {
    ...
    const pagination = useMemo(() => data?.pagination || { page: 1, limit, total: 0, pages: 1 }, [data, limit]);
    ...
    const openDeleteModal = useCallback((admin) => setDeleteModal({ isOpen: true, admin }), []);
    const closeDeleteModal = useCallback(() => setDeleteModal({ isOpen: false, admin: null }), []);
