...
import { useRequests, useApproveRequest, useRejectRequest } from './useRequests';
...
export function useRequestsPage() {
    ...
    const { data, isLoading: loading, refetch } = useRequests({
        page,
        limit,
    ...
    const pagination = data?.pagination || { page: 1, limit: 10, total: requests.length, pages: 1 };

    const handleApprove = useCallback((req, onSuccess) => {
        notify.promise(
    ...
    const handleReject = useCallback((reqId, reason, onSuccess) => {
        notify.promise(
            new Promise((resolve, reject) => {
                rejectRequest(
                    { id: reqId, reason },
