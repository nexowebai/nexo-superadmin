...
import { AreaChart, Area, Grid, XAxis, YAxis, ChartTooltip } from '@/components/ui/AreaChart';
        ...
        <Card variant="pro" padding="lg" className="financial-performance-card">
            <div className="financial-header">
                <div className="financial-header__info">
                    ...
                    <div>
                        ...
                        <p className="financial-header__subtitle">Track your earnings and trends</p>
                    </div>
                </div>

                <div className="financial-header__action">
                    <DatePicker value={selectedDate} onChange={setSelectedDate} className="w-[180px]" />
                </div>
            </div>
            ...
            <div className="financial-chart">
                <AreaChart data={chartData} aspectRatio="3.5 / 1">
                    ...
                    <Area dataKey="revenue" fill="var(--primary)" fillOpacity={0.08} strokeWidth={2.5} fadeEdges />
                    <Area dataKey="costs" fill="#38bdf8" fillOpacity={0.04} strokeWidth={2} fadeEdges />
                    <YAxis numTicks={5} formatValue={(v) => `$${(v / 1000).toFixed(0)}K`} />
                    <XAxis numTicks={6} />
                    <ChartTooltip
                        rows={(point) => [
                            { color: "var(--primary)", label: "Revenue", value: `$${Math.round(point.revenue).toLocaleString()}` },
                            { color: "#38bdf8", label: "Costs", value: `$${Math.round(point.costs).toLocaleString()}` },
                        ]}
                    />
                </AreaChart>
            </div>
