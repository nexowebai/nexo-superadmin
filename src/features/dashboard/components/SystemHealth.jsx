  ...
  return (
    <div className="relative w-56 h-56 flex items-center justify-center">
      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 192 192">
        <circle cx="96" cy="96" r={radius} stroke="var(--border-base)" strokeWidth="12" fill="transparent" className="opacity-20" />
        <motion.circle
          cx="96" cy="96" r={radius} stroke="url(#healthGradient)" strokeWidth="12" strokeDasharray={circumference}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
        <motion.span initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} className="text-4xl font-black text-[var(--text-primary)] tracking-tighter">
          {score}%
        </motion.span>
        <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest mt-1">Global Health</span>
      </div>
    </div>
  );
}
