...
export function useOrganizations(params = {}) {
  return useQuery({
    ...
    select: (response) => ({
      organizations:
        response?.data?.organizations || response?.organizations || [],
...
export function useEnableOrganization() {
  ...
  return useMutation({
    ...
    onError: (err) =>
      notify.error(err?.message || "Failed to enable organization"),
  });
}
...
export function useDisableOrganization() {
  ...
  return useMutation({
    ...
    onError: (err) =>
      notify.error(err?.message || "Failed to disable organization"),
  });
}
...
export function useResetOrgPassword() {
  return useMutation({
    mutationFn: ({ id, password }) =>
      orgService.resetAdminPassword(id, password),
