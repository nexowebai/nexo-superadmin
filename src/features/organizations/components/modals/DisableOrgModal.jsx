...
export default function DisableOrgModal({ isOpen, onClose, orgId, orgName, onSuccess }) {
      ...
      <form onSubmit={handleDisable}>
        <ModalHeader className="modal-form-header">
          ...
          <div className="modal-form-header__content">
            ...
            <ModalDescription>Confirming this action for <span className="font-bold text-primary">{orgName}</span></ModalDescription>
          </div>
        </ModalHeader>
        ...
        <ModalBody>
          <div className="bg-elevated p-4 rounded-lg border border-base space-y-3">
            <p className="text-sm text-secondary leading-relaxed">
              Disabling this organization will immediately revoke access for all its users. They will not be able to log in until the organization is re-enabled.
            </p>
            <ul className="space-y-3 pt-1">
              <WarningItem label="All active user sessions will be terminated immediately." />
              <WarningItem label="Automated background processes and tasks will be paused." />
            </ul>
          </div>
        </ModalBody>
...
function WarningItem({ label }) {
  return (
    <li className="flex items-center gap-3 text-sm disable-warning-item">
      <div className="w-1 h-1 rounded-full shrink-0 disable-warning-dot" />
      {label}
    </li>
  );
}
