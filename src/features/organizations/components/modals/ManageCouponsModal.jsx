            ...
            <form onSubmit={(e) => { e.preventDefault(); onClose(); }}>
                <ModalHeader className="modal-form-header">
                    ...
                    <div className="ds-modal-header__content">
                        ...
                        <ModalDescription>Configure and assign discounts for <span className="font-bold text-primary">{orgName}</span></ModalDescription>
                    </div>
                </ModalHeader>
                <ModalBody className="space-y-6">
                    <div className="flex items-center justify-between">
                        <div className="section-title-nx">
                            <h4 className="text-xs font-bold uppercase tracking-widest text-muted">Active Assignments</h4>
                        </div>
                    ...
                    <div className="border border-base rounded-lg overflow-hidden bg-surface">
                        <table className="w-full text-sm border-collapse">
                            ...
                            <tbody className="divide-y divide-base">
                                {coupons.length === 0 ? (
                                    <tr>
                                        <td colSpan={4} className="p-12 text-center text-muted text-xs italic">No coupons assigned</td>
                                    </tr>
                                ) : (
                                    coupons.map(c => (
                                        <tr key={c.id} className="hover:bg-bg-elevated transition-colors border-0">
                                            <td className="p-4 font-mono font-bold text-primary">{c.code}</td>
                                            <td className="p-4 font-medium text-secondary">{c.discount}</td>
                ...
                <ModalFooter className="ds-confirm-modal__footer">
                    <Button variant="ghost" onClick={onClose} type="button" className="ds-confirm-modal__cancel w-full">Close</Button>
                </ModalFooter>
            </form>
        </Modal>
    );
